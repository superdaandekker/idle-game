...
// Icons for click and offline upgrades only
const UPGRADE_ICONS = {
    "better-finger": "BF",
    "iron-mouse": "IM",
    "espresso-shot": "ES",
...
// Only click and offline upgrades remain — passive income is handled by BUILDING_DEFINITIONS above.
...
function getVisibleUpgrades() {
    return game.upgrades.filter((upgrade) => {
        const def = UPGRADE_DEFINITIONS.find((d) => d.id === upgrade.id);
        if (!def) return false;
        if (game.lifetimeEarned < (upgrade.unlockAt || 0)) return false;
        return true;
    });
}
...
function getBulkPurchaseInfo(upgrade) {
    ...
    if (game.buyAmount === "max") {
        ...
        while (game.money >= totalCost + nextCost) {
            totalCost += nextCost;
            amount += 1;
    ...
    const requestedAmount = Math.max(1, Number(game.buyAmount) || 1);
    let totalCost = 0;
    ...
    return { amount: requestedAmount, totalCost };
}
...
function recalculateDerivedStats() {
    ...
    // Accumulate click bonuses from click upgrades
    ...
    // Offline bonus from offline upgrades
    game.offlineBonus = game.upgrades.reduce((sum, u) => u.effectType === "offline" ? sum + u.amountOwned * u.effectValue : sum, 0);

    // Building income (passive income is fully handled by BUILDING_DEFINITIONS)
    ...
    game.clickValue = (getBaseClickValue() + clickBonus) * game.prestigeBonus * clickEventMultiplier * boostActive;
    game.moneyPerSecond = passiveIncome * game.prestigeBonus * passiveEventMultiplier * boostActive;
...
function migrateSave(rawSave) {
    ...
    return {
        ...
        // Migrate old "passive" subtab to "businesses"
        activeSubTab: (() => {
            const t = parsedSave.activeSubTab;
            if (t === "passive") return "businesses";
            if (["click", "businesses", "offline", "premium"].includes(t)) return t;
            return "businesses";
        })(),
        ...
        // Only keep click and offline upgrades from saved data; discard old passive/buildingMultiplier entries
        upgrades: Array.isArray(parsedSave.upgrades)
            ? parsedSave.upgrades.filter(u => UPGRADE_DEFINITIONS.some(d => d.id === u.id))
            : [],
...
function applySaveData(saveData, options = {}) {
    ...
    game.activeSubTab = saveData.activeSubTab || "businesses";
...
function setActiveSubTab(name) {
    if (!["click", "businesses", "offline", "premium"].includes(name)) return;
...
function renderClickShop(container) {
    ...
    const upgrades = getVisibleUpgrades().filter(u => u.effectType === "click");
...
function renderOfflineShop(container) {
    ...
    const upgrades = getVisibleUpgrades().filter(u => u.effectType === "offline");
